...
DARK=make_mat("V3_Dark",(0.010,0.018,0.036,1),0.72,0.24)
MID=make_mat("V3_Mid",(0.045,0.075,0.13,1),0.78,0.20)
ORANGE=make_mat("V3_Orange",(1.0,0.10,0.012,1),0.30,0.18)
BLUE=make_mat("V3_Blue",(0.005,0.10,1.0,1),0.92,0.10)
GREEN=make_mat("V3_Green",(0.005,0.24,0.025,1),0.18,0.15,(0.01,1.0,0.07,1),5.0)
CYAN=make_mat("V3_Cyan",(0.005,0.10,0.16,1),0.2,0.16,(0.01,0.55,1.0,1),2.5)
...
for o in list(bpy.data.objects):
    if o.name.startswith(("V2_","V3_","Hub_","Status_","Entry","Machine","Portal","SideWall","Deck")):
...
for i,x in enumerate(xs):
    ...
    hub=bpy.context.object; hub.name=f"V3_Hub_{i+1}"; hub.data.materials.append(MID)

    bpy.ops.mesh.primitive_torus_add(major_radius=.22,minor_radius=.035,
        location=(x,-.52,.18),rotation=(math.radians(90),0,0))
    led=bpy.context.object; led.name=f"V3_Status_{i+1}"; led.data.materials.append(CYAN)

# Entrance collar around the blue marble path.
cube("V3_EntryTop",(-3.60,0,2.10),(1.15,1.0,.08),MID,.06)
cube("V3_EntryL",(-4.62,-.88,1.18),(.08,.08,.92),MID,.04)
cube("V3_EntryR",(-4.62,.88,1.18),(.08,.08,.92),MID,.04)
...
for radius,minor in [(0.88,.075),(0.64,.045)]:
    ...
    o=bpy.context.object; o.name="V3_PortalRing"; o.data.materials.append(GREEN)

bpy.ops.mesh.primitive_cylinder_add(vertices=48,radius=.34,depth=.16,
    location=(portal_x,.10,.58),rotation=(math.radians(90),0,0))
lock=bpy.context.object; lock.name="V3_PortalCore"; lock.data.materials.append(GREEN)
lock.scale=(.86,.86,.86); lock.keyframe_insert(data_path="scale",frame=60)
lock.scale=(1.28,1.28,1.28); lock.keyframe_insert(data_path="scale",frame=78)
lock.scale=(1.0,1.0,1.0); lock.keyframe_insert(data_path="scale",frame=90)

# Two portal doors: readable payoff instead of one green slab.
left=cube("V3_DoorL",(3.48,-.47,.58),(.07,.35,.55),MID,.05)
right=cube("V3_DoorR",(3.48,.47,.58),(.07,.35,.55),MID,.05)
left.rotation_euler=(0,0,0); right.rotation_euler=(0,0,0)
left.keyframe_insert(data_path="rotation_euler",frame=60)
right.keyframe_insert(data_path="rotation_euler",frame=60)
left.rotation_euler=(0,math.radians(-58),0)
right.rotation_euler=(0,math.radians(58),0)
left.keyframe_insert(data_path="rotation_euler",frame=84)
right.keyframe_insert(data_path="rotation_euler",frame=84)
left.keyframe_insert(data_path="rotation_euler",frame=90)
right.keyframe_insert(data_path="rotation_euler",frame=90)
...
area("V3_WarmKey",(-2.6,-4.5,6.8),1900,4.6,(1.0,.72,.50))
area("V3_Fill",(3.5,-2.2,4.5),1250,4.2,(.45,.62,1.0))
area("V3_Rim",(0.8,3.8,5.8),1450,3.2,(.38,.70,1.0))
area("V3_GoalGlow",(3.0,-1.2,2.0),700,2.0,(.25,1.0,.40),target=(2.95,0,.58))
...
for fno,name in [(60,"v3_action.png"),(90,"v3_payoff.png")]:
    scene.frame_set(fno)
    scene.render.filepath=os.path.join(outdir,name)
    bpy.ops.render.render(write_still=True)

with open(os.path.join(outdir,"v3_meta.txt"),"w") as f:
    f.write("V3_PERSPECTIVE_MACHINE 270x480 frames 60,90 five-paddle\n")
